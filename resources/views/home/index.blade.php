...
<div class="page-blue">
    ...
    {{-- TRAVELING --}}
    <div class="section-title">TRAVELING</div>
    <div class="row g-3">
        @forelse($traveling as $hotel)
        <div class="col-4">
            <a href="{{ route('hotel.detail', $hotel) }}" class="travel-card">
                @if($hotel->image)
                    <img src="{{ asset('storage/' . $hotel->image) }}" alt="{{ $hotel->name }}">
                @else
                    <div class="placeholder-img placeholder-img-travel"><i class="bi bi-geo-alt"></i></div>
                @endif
                <div class="card-label">{{ strtoupper($hotel->name) }}</div>
            </a>
        </div>
        @empty
        <div class="col-12 text-center text-white opacity-50 py-4">
            <i class="bi bi-geo-alt" style="font-size:40px;"></i>
            <p class="mt-2">Belum ada destinasi traveling.</p>
        </div>
        @endforelse
    </div>
